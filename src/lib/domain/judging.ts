...
export function weightedScore(scores: Record<string, number>) {
  const total = rubric.reduce((sum, item) => sum + (scores[item.id] ?? 0) * item.weight, 0);
...
export function detectScoreDrift(records: ScoreRecord[]) {
  ...
  const means = [...judgeAverages.entries()].map(([judgeId, values]) => ({ judgeId, mean: values.reduce((a, b) => a + b, 0) / values.length, samples: values.length }));
  const populationMean = means.reduce((sum, item) => sum + item.mean, 0) / means.length;
  const deviation = Math.sqrt(means.reduce((sum, item) => sum + (item.mean - populationMean) ** 2, 0) / means.length);
  if (deviation === 0) return [];

  return means
    .filter((item) => item.samples >= 3 && Math.abs(item.mean - populationMean) / deviation > 1.5)
    .map((item) => ({ judgeId: item.judgeId, delta: Math.round((item.mean - populationMean) * 10) / 10, samples: item.samples }));
}
